...
;(function($){
	function tabPlugin(opts,$ele) {
		...
		for(var i =0; i< this.opt.data.length;i++) {
			this.frameArr.push(this.opt.data[i]);
		}
	}
	...
	tabPlugin.prototype = {
		...
		_createWrap: function(){
			...
			for(var i = 0;i<this.opt.data.length;i++) {
				...
				if(itemData.del || itemData.del == void(0)) {
		...
		_navButton: function(){
			...
			$(document).on("click",".SysPageNavDownBar li",function(){
				...
				//index:1 关闭全部
				function cloeseAll() {
					$(".SysPagesList  a").eq(0).addClass("SysPageActive").siblings().remove();
					$(".SysFrameList iframe").eq(0).removeClass('SysHide').siblings().remove();
					that.frameArr = [that.frameArr[0]];
					debugger;
